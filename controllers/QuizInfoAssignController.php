...
require_once 'dao/T_QuizInfoAssignDao.php';
require_once 'service/QuizInfoAssignService.php';
 ...
 * テスト・クイズ割当コントローラー
 */
class QuizInfoAssignController extends BaseController {
	/**
	 * 初期処理
	 */
	public function indexAction() {
		...
		if ($this->check_login() == true){
			...
			$this->smarty->display ( 'quizInfoAssign.html' );
		} else {
	...
	private function getTestData(){
		...
		$service = new QuizInfoAssignService($this->pdo);
		$testList = $service->getTestData($this->form->org_no, $test_info_no);
		if( count($testList) > 0 ){
	...
	private function search() {
		...
		if(count($quizList) > 0){
			...
			$registerList = $service->getRegisteredQuizList($this->form->org_no, $this->form->test_info_no);

			if(count($registerList) > 0){
				//get register quiz_info_no list
				foreach ($registerList as $value){
					array_push($registerQuizList, $value->quiz_info_no);
				}

				foreach ($quizList as $value){

					if(in_array($value->quiz_info_no,$registerQuizList)){
	...
	public function saveAction() {
		...
		if ($this->check_login () == true) {
			...
			foreach ( $insertDataList as $insertData ) {
				...
				if ($insertData != "") {
					$test_Info_quizDto = new T_Test_Info_QuizDto ();
					$test_Info_quizDto->org_no = $org_no;
					$test_Info_quizDto->test_info_no = $test_info_no;
					$test_Info_quizDto->quiz_info_no = $insertData;
					$test_Info_quizDto->disp_no = ++$display_no;
					$test_Info_quizDto->del_flg = '0';
					...
					$result = $service->addQuizDataOnTest ( $test_Info_quizDto );
					if ($result == 0){
