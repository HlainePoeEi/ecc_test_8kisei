...
$(document).ready(function() {
	...
	$('#testAss_lowertbl1 > tbody tr td:nth-child(6)').hide();
	...
	if(newlower_tbl.rows.length <3){
		for (var j = 1; j <= newlower_tbl.rows.length; j++){
			newlower_tbl.rows[j].cells[0].innerHTML = 1;
				newlower_tbl.rows[j].cells[4].innerHTML = '<button style="background-color:#fff;border:0px;" disabled="disabled" onClick="MoveUp.call(this);return false;"'
						+ '><img src="'
		...
		for (var j = 1; j < newlower_tbl.rows.length; j++) {
			if(j == 1){
				...
				newlower_tbl.rows[j].cells[0].innerHTML = j;
				newlower_tbl.rows[j].cells[4].innerHTML = '<button style="background-color:#fff;border:0px;" onClick="MoveUp.call(this);return false;"'
						+ '><img src="'
						+ homeDir + 'image/up.svg'
						+ ' "style="width:25px;height:25px;"/></a><button style="background-color:#fff;border:0px;" disabled="disabled" onClick="MoveDown.call(this);return false;"'
						+ '><img src="'
 ...
 */
function insertQuizAssignmentData() {
	// formのデータを取得する
	var homeDir = $('#home_dir').val();
...
function readLowerTbl() {
	...
	if (lower_tbl.rows.length > 1) {
		for (var j = 1; j < lower_tbl.rows.length; j++) {
			if (j == 1) {
				tblDataArrStr = lower_tbl.rows[j].cells[5].innerHTML;
			} else
				tblDataArrStr += "," + lower_tbl.rows[j].cells[5].innerHTML;
		}
	}
